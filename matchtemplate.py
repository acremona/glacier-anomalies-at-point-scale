...
def clean_scales(daily_disp, conversion_factors, boxlength):
    """
    Makes the scale array smoother by applying a convolution function.

    Parameters
    ----------
    daily_disp : list of tuples
        List of all calculated displacements (not cumulative) and the index of the cumulative value it should be added to to build a cumulative array.
    conversion_factors : list of float
        List of all conversion factors for every image. Same length as daily_disp
    boxlength
        Amount of values that should be considered for building a floating average. A higher number yields smoother results. Recommended and tested value: 100. Value must not be bigger than half of the length of conversion_factors.
    Returns
    -------
    clean_displacements : list of float
        cumulative displacements calculated with new smooth scaling factors
    scale_smooth : list of float
        list of convoluted conversion factors
    """
    clean_displacements = []
...
def matchTemplate_hist(folder_path, template_path, thresh, wait, vis=False, plotting=False):
    """
    Calculates total displacements for a given series of images.

    Parameters
    ----------
    original_images : list of array
        List of all images. Usually generated by load_images_from_folder function.
    times : list of
    template_path
    thresh
    wait
    ...
    global h, w, rough_scale, threshTracking, threshDuplicate, threshAngle, frame_nr, h_tot, w_tot, x, res_img, visualize, times
    ########################################################################################################
    ...
    ########################################################################################################
    original_images, times = load_images_from_folder(folder_path)
    visualize = vis
    ...
    plt.plot(x, all_scales, label="Average (n=200) + convolution (n=100)")
...
x, total_displacement, scales = matchTemplate_hist("C:\\Users\\joelb\\Downloads\\holfuy_images_2019\\1001", template_path, 0.69, 1)
plt.plot(x, scales, label="Average (n=200)")
